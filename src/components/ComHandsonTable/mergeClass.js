    ...
    for (let i = 0; i < dataList.length; i++) {
      for (let j = 0; j < countField.length; j++) {
        if (i < dataList.length - 1) {
          if (
            dataList[i][countField[j].field] ==
              dataList[i + 1][countField[j].field] &&
            dataList[i][countField[j].field] &&
            dataList[i + 1][countField[j].field]
          ) {
            mergeList[countField[j].field][
              mergeList[countField[j].field].length - 1
            ].len++;
          } else {
            let das =
              mergeList[countField[j].field][
      ...
      if (countField[i].children && countField[i].children.length > 0) {
        ...
        for (let j = 0; j < list.length; j++) {
          for (let z = 0; z < countField[i].children.length; z++) {
            ...
            if (
              mergeList[filedChild].length == 1 &&
              mergeList[filedChild][0].index == 0 &&
              mergeList[filedChild][0].len == 1
            ) {
            ...
            let dataChildList = JSON.parse(JSON.stringify(dataList)).splice(
              list[j].index,
              list[j].len
            );
