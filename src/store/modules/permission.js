  ...
  var find = function (arr, id) {
    ...
    if (!id) {
      var newArr = arr.filter(
        (m) => m.ParentCode == id || m.ParentCode == null
      );
    } else {
    ...
    newArr.some((item, o) => {
      ...
      newBtns.forEach((x) => {
        if (
          allBtns.findIndex((y) => {
            return y.ButtonCode == x.ButtonCode;
          }) == -1
        ) {
          allBtns.push(x);
        }
      ...
      let childmodel = {
        ...
        meta: {
          ...
          affix: item.MenuName == "首页" ? true : false,
...
const actions = {
  GenerateRoutes({ commit }) {
    ...
    if (
      role.findIndex((r) => {
        return r.Account == "admin" || r.Account == "378102";
      }) != -1
    ) {
      asyncRouterMapNew = asyncRouterMapNew.concat(systemSetting);
    }
    // 菜单
    asyncRouterMapNew = asyncRouterMapNew.concat(
      GetMenuList(store.getters.menus, "")
    );
    // 把首页永远排在首位
