...
const actions = {
  ...
  login({ commit }, userInfo) {
    ...
    return new Promise((resolve, reject) => {
      NEWLOGIN({ Pwd: Pwd, Account: Account, eipaulg: '' }).then(res => {
        if (res.data.result) {
          let obj = { Pwd: res.data.dev_Account.Pwd, Account: res.data.dev_Account.Account, UserName: res.data.Name, MFGOrganizeID: res.data.dev_Account.CenterID };
  ...
  autoLogin({ commit }, userInfo) {
    ...
    return new Promise((resolve, reject) => {
      NEWLOGIN({ Pwd: Pwd, Account: Account, eipaulg: eipaulg }).then(res => {
        if (res.data.result) {
          let obj = { Pwd: res.data.dev_Account.Pwd, Account: res.data.dev_Account.Account, UserName: res.data.Name, MFGOrganizeID: res.data.dev_Account.CenterID };
          localStorage.setItem('userInfo', JSON.stringify(obj));
          commit('SET_TOKEN', res.data.token);
    ...
    return new Promise((resolve, reject) => {
      refreshToken(state.token).then(response => {
        ...
        if (code == 20000) {
          ...
          sessionStorage.setItem("tokenExpire", formatDate() + " " + date.toTimeString().split(" ")[0])
  ...
  userInfo({ commit, dispatch }, userInfo) {
    const { Pwd, Account } = userInfo
    return new Promise((resolve, reject) => {
      NEWLOGIN({ Pwd: Pwd, Account: Account }).then(res => {
