...
(function (root, factory) {
  ...
  if (major < 2 || (major === 2 && minor < 6)) {
    throw new Error(
      "moment-msdate requires Moment.js >= 2.6.0. You are using Moment.js ${moment.version}. See momentjs.com"
    );
  }

  if (!momentTimezone || !moment.tz) {
    throw new Error(
      "moment-msdate requires moment-timezone.js. see momentjs.com/timezone"
    );
  }

  var oaDateToTicks = function (oaDate) {
    return (
      (oaDate - MS_DAY_OFFSET) * DAY_MILLISECONDS + (oaDate >= 0.0 ? 0.5 : -0.5)
    );
  };
  ...
  var fromOADateOffsetToUtcByTimezone = function (oaDate, timezone) {
    if (!moment.tz.zone(timezone)) {
      throw new Error(
        "timezone provided is not available in moment-timezone.js",
        "moment-msdate.js",
        59
      );
    }
