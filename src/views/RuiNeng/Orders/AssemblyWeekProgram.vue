...
export default {
  ...
  methods: {
    ...
    setData(remarkTb) {
      ...
      this.spread[remarkTb]
        .commandManager()
        .register("insertRowsCut", insertRowsCut);
    ...
    async MOPlanSaveToDayPlan() {
      ...
      if (this.selectionData[this.labelStatus2].length == 0) {
        this.$message.error("请选择需要更新的计划！");
        return;
      }
      let submitData = this.selectionData[this.labelStatus2];
      let errorArray = submitData.filter(item => {
        item["FormRate"] !== "100%";
      });
