  ...
  watch: {
    $route: {
      handler: function (val, oldVal) {
        if (val.params.hasOwnProperty("row")) {
          this.parseSearch = true;
          this.parseForm = JSON.parse(val.params["row"]);
          this.$set(
            this.formSearchs[0].datas,
            "OrderID",
            this.parseForm.OrderID
          );
          this.$set(this.formSearchs[0].datas, "dicID", 6748);
          this.dataSearch(0);
        } else {
