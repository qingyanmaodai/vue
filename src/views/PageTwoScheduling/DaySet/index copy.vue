...
export default {
  ...
  methods: {
    ...
    setData() {
      ...
      this.tableColumns[0].forEach((x) => {
        if (!x.children) {
          ...
          colInfos.push({ name: x.prop, displayName: x.label });
        } else {
      ...
      // 下拉框
      const checkBoxCellType = new GCsheets.CellTypes.ComboBox();
      let options = [
        { text: "1线", value: "1" },
        { text: "2线", value: "2" },
        { text: "3线", value: "2" },
      ...
      // checkBoxCellType.editorValueType(GCsheets.CellTypes.EditorValueType.text);
      checkBoxCellType.items(options);
      checkBoxCellType.itemHeight(24);
      let obj = {
    ...
    allowDragMerge(e) {
      this.spread.options.allowUserDragMerge =
        document.getElementById("allowDragMerge").checked;
    },
    //////////////////////////////
    // 获取组织
    async getOrgData() {
      ...
      if (result) {
        ...
        if (data.length != 0) {
          ...
          // this.formSearchs[0].datas["WorkShop"] = data[0].OrganizeID;
        }
      } else {
        this.$message({
          message: msg,
