...
export default {
  ...
  methods: {
    ...
    // 高度控制
    setHeight() {
      this.treeHeight = document.documentElement.clientHeight - 100 + "px";
      ...
      this.$confirm("是否确定审核?")
        .then(async () => {
          ...
          let res = await GetSearch(
            ...
            "/APSAPI/UpdateOrderStartDate?isUpdate=1"
          );
          const { result, data, count, msg } = res.data;
          if (result) {
            this.selectionData[this.tagRemark] = [];
            ...
            this.adminLoading = false;
            this.$set(this.btnForm[this.tagRemark][index], "isLoading", false);
          } else {
            this.$message({
              message: msg,
              type: "error",
