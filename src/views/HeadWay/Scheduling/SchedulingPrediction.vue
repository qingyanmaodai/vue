...
<template>
  ...
  export default {
    ...
    created() {
      ...
      // 计算周期默认时间：今天~1.5月
      _this.machineCycle = [formatDates.formatTodayDate(), formatDates.formatOneMonthDate()];
    },
    mounted() {
      setTimeout(() => {
        this.setHeight();
        ...
        if (importData && importData.length > 0) {
          ...
          importData[0].sheet.forEach((m,y) => {
              ...
              for (let i = 0; i < this.tableColumns[this.tagRemark].length; i++) {
                ...
                if (item.label === key) {
                  if (item.DataType === "datetime") {
                    ...
                    // 判断需求到料日期是否大于今天
                  if(item.prop==='PlanDay'&&obj[item.prop]<formatDates.formatTodayDate()){
                      propName = key
                      rowNo =Number(m.__rowNum__)+1
                      // 异常提示
            ...
            if (!isDate) {
              ...
              obj["SDate"] =  _this.machineCycle.length?_this.machineCycle[0]:'',
              obj["Edate"]  = _this.machineCycle.length?_this.machineCycle[1]:'',
