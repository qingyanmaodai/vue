<template>
  ...
  >
    <el-container class="home">
        ...
        <div class="home_5">
          <div class="home_6">
            ...
            <div class="home_12">
              ...
              <div class="home_14" style="overflow-y: auto;">
                <div class="balance" v-for="(item,i) in proceList">
                  <i
                    class="home_dot"
                    :style="{background: color[i]}"
                  />
                  <span>{{item.ProcessName}}计划数</span>
                  <span style="float:right">{{ item.PlanQty }}</span>
                </div>
                <!-- <div class="balance">
                  <i
                    class="home_dot"
      ...
      >
        <div class="home_28">
          <span class="home_right_2">待办事项</span>
          <div
            class="asideOne"
            @click="toPage('BadAduit',dotPage[0].isUsing)"
          >
            <i
              class="home_dot"
          ...
          <div
            class="asideOne"
            @click="toPage('BadAduit',dotPage[1].isUsing)"
          >
            <i
              class="home_dot"
          ...
          <div
            class="asideOne"
            @click="toPage('BadAduit',dotPage[2].isUsing)"
          >
            <i
              class="home_dot"
...
export default {
  ...
  methods: {
    ...
    async getDataOne() {
      ...
      form["fields"] ='ProcessName,sum(PlanQty) as  PlanQty';
      form["groupby"] = "ProcessName";
      form["sort"] = "ProcessName";
      form['rows'] = 0
      ...
      if (result) {
        if (data.length != 0) {
          ...
          this.proceList = data
          this.proceList.forEach((item)=>{
            item['name'] = item.ProcessName
            item['value'] = item.PlanQty
          })
          this.GettodayData(this.proceList)
          // this.pieForm = data[0];
    ...
    async getWjBoardData_1() {
      let form = {};
      form["OrganizeIDs"] = this.userInfo.CenterID;
      form["PlanDay"] = [formatDate.formatOldDate(), formatDate.formatOldDate()];
      form["dicID"] = 6723;
      form["rows"] = 0;
      form["fields"] =
      ...
      if (result) {
        ...
        if (data.length != 0) {
          for (var i = 0; i < data.length; i++) {
            ...
            PlanQtyP.push(data[i].ConfirmQty?(parseFloat((data[i].ConfirmQty/data[i].PlanQty)*100).toFixed(2)):0);
          }
    ...
    async getEchartTwoData() {
      ...
      // form["groupby"] = " ProcessName";
      // form["dicID"] = 6720;
      form["fields"] ="OrganizeName,sum(BadQty) as BadQty,AbnormalReason";
      form["CreatedOn"] = [formatDate.formatOldDate(), formatDate.formatOldDate()];
      form["groupby"] = "OrganizeName,AbnormalReason";
      form["dicID"] = 7811;
      form["rows"] = 0;
      let res = await GetSearchData(form);
      const { result, data, count, msg } = res.data;
    ...
    GettodayData(data) {
      var echarts = require("echarts");
      const myCharts1 = echarts.init(document.getElementById("todayOption"));
      this.todayOption.series[0].data = data
      myCharts1.setOption(this.todayOption);
    },
    async getOweQtyData(){
      let form = {};
      form["SDate"] =formatDate.formatTodayDate();
      form["Edate"] = formatDate.formatOneMonthDate();
      form["OweQty"] = 0;
      form["dicID"] = 9006;
      form["rows"] = 0;
      let res = await GetSearchData(form);
