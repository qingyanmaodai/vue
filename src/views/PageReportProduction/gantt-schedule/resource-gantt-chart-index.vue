      ...
      <div>
        <div  class="admin_content">
          <div class="gstc-wrapper" id="gstc" ref="gstc">
            <div slot="header" class="ganttHeader" style="display: flex;align-items: center;">
              <!-- 高度滑块 -->
              <div style="margin-right: 20px;display: flex;flex-direction: row;align-items: center;">
                <span>单元格高度</span>
                <el-slider v-model="customHightSize" :min="30" :max="100" :step="10" :show-tooltip="false" @input="changeHeight" style="width: 200px;margin-left: 10px;"></el-slider>
              </div>
              <!-- 宽度滑块 -->
              <div style="margin-right: 20px;display: flex;flex-direction: row;align-items: center;">
                <span>单元格宽度</span>
                <el-slider v-model="customWidthSize" :min="0" :max="4" :step="0.5" :show-tooltip="false"  @input="changeWidth" style="width: 200px;margin-left: 10px;"></el-slider>
              </div>
            </div>
          </div>
          <div>
...
function setTippyContent(element, data) {
  ...
  if (!element._tippy) return;
    const tooltipContent =`
      ...
      <p>工段名称：${data.item.ProcessPartName ? data.item.ProcessPartName : ""}</p>
  ...
  methods: {
                    ...
                    for(let x=0;x<children.length;x++){
                      ...
                      let itemsObj = {
                        [num]: {
                          ...
                          OrderNo: children[x].OrderNo,
                          StartDate:children[x].StartDate?self.$moment(children[x].StartDate).format("YYYY-MM-DD"):'',
                          StartTime:children[x].StartTime?self.$moment(children[x].StartTime).format("YYYY-MM-DD HH:mm:ss"):'',
                          EndTime:children[x].EndTime?self.$moment(children[x].EndTime).format("YYYY-MM-DD HH:mm:ss"):'',
                          time: {
    ...
    async getTableHeader() {
      ...
      if (result) {
        ...
        this.formSearchs[this.tagRemark].datas['StartDate'][0] = this.weekStartDate
        this.formSearchs[this.tagRemark].datas['StartDate'][1] = this.weekEndDate
