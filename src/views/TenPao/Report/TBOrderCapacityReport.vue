...
export default {
  ...
  methods: {
    ...
    findColumn(columns, targetColumn) {
      for (let i = 0; i < columns.length; i++) {
        const column = columns[i];
        if (column['prop'] === targetColumn['property'] && column['prop2']) {
          return column;
        }
        if (column.children && column.children.length > 0) {
          const childMatchedColumn = this.findColumn(
            column.children,
            targetColumn,
          );
          if (childMatchedColumn) {
            return childMatchedColumn;
          }
        }
      }
