...
export default {
  ...
  methods: {
    ...
    async getTableHeader() {
      ...
      if (result) {
        ...
        forms.some((x, z) => {
          ...
          // this.formSearchs[0].datas["HasLine"] = ''
          this.getLineData();
        });
      } else {
        this.adminLoading = false;
    ...
    handleNodeClick(data, node) {
      this.clickData = data;
      this.$set(this.formSearchs[0].datas, "ControlID", data.ERPOrderCode);
      this.getLineData(data.OrganizeID);
      this.tableLoading[0] = true;
    },
    // 下达
    async setOrderToDay(remarkTb, index, MOSchedulingType) {
      ...
      this.adminLoading = true;
      let res = await SalesPlanProcessMaterialDay(newData);
      const { result, data, count, msg } = res.data;
      if (result) {
        this.dataSearch(0);
        this.adminLoading = false;
    ...
    async autoLine() {
      ...
      if (this.selectionData.length == 0) {
        this.$message.error("请勾选要自动匹配选的数据！");
      } else {
        this.adminLoading = true;
        let res = await GetSearch(this.selectionData, "/APSAPI/GetHWPlanLine");
        const { result, data, count, msg } = res.data;
        this.adminLoading = false;
        // 将返回的数据重新赋值
        data.some((x, i) => {
    ...
    changeStatus(item, index) {
      ...
      this.formSearchs[0].datas["HasLine"] = item.value;
