  ...
  methods: {
    ...
    async dataSave(remarkTb, index, parms) {
      ...
      if (parms && parms.dataName) {
        if (this[parms.dataName][remarkTb].length == 0) {
          ...
          res = await SaveData(this[parms.dataName][remarkTb]);
        }
      } else {
    ...
    async dataDel(remarkTb, index, parms) {
      ...
      if (parms && parms.dataName) {
        ...
        res = await SaveData(newData, remarkTb, index);
      }
    ...
    async getTableData(form, remarkTb) {
      ...
      if (result) {
        if (res.data.Columns && res.data.Columns.length != 0) {
          res.data.Columns[0].some((n, i) => {
            this.verifyDta(n);
          });
          this.$set(this.tableColumns, remarkTb, res.data.Columns[0]);
        }
