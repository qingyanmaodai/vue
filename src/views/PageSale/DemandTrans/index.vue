...
export default {
  ...
  methods: {
    ...
    async dataSave(remarkTb, index) {
      ...
      let realData = jspreadsheettable[0].getJson();
      if (realData.length == 0 || realData.length == 1) {
        this.$message.error("没有需要更新的数据!");
        return;
      } else {
        let setData = jspreadsheettable[0].getJson(); // 给0是为了判断是保存，每个数据都加上dicID
    ...
    async getTableHeader() {
      // 6679
      let IDs = [{ ID: 6679 }, { ID: 6699 }, { ID: 6670 }];
    ...
    setSpreadSheet(data) {
      ...
      document.getElementById("specialSheet").innerHTML = "";
      jspreadsheettable[0] = jspreadsheet(
        document.getElementById("specialSheet"),
        {
    ...
    async analySis(remarkTb, index) {
      this.adminLoading = true;
      let res = await SysData(jspreadsheettable[0].getJson());
