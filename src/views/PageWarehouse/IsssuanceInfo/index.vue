...
<template>
  <div class="container" v-loading="adminLoading">
    ...
    >
      ...
      <el-container>
        <span class="content_head">出库单</span>
        <GeneralForm
          class="formStyle"
          ref="ruleForm0"
          :rule-form="ruleForm0"
        ...
        <el-main>
          ...
          />
          <el-input type="textarea" placeholder="暂无备注信息" rows="2" />
          <span class="footer_span">制单人：</span>
        </el-main>
    ...
    <div :class="showBox ? 'active_box dialog_box' : 'error_box dialog_box'">
      ...
      <div class="box_footer">
        <el-button class="green_btn" size="small" type="" @click="updateStock"
          >更新库存</el-button
        >
      </div>
      <lodop ref="lodop" />
...
export default {
  ...
  data() {
    return {
      ...
      rules0: {
        DeliveryPlanName: [
          { required: true, message: "出货日期必填！", trigger: "change" },
        ],
        // CustomerID: [
  ...
  mounted() {
    setTimeout(() => {
      this.setHeight();
      this.tableHeight2 = document.documentElement.clientHeight - 520 + "px";
    }, 450);
  },
  methods: {
    // 获取客户
    ...
    async updateStock() {
      ...
      debugger;
      if (this.tableData[2] && this.tableData[2].length != 0) {
        res = await SaveData(this.tableData[2]);
      } else {
        this.$message({
          message: "没有要更新的数据",
          type: "error",
    ...
    async editRow(row) {
      ...
      let res = await GetSearchData({
        dicID: 1201,
        DeliveryPlanHeadID: row.DeliveryPlanHeadID,
      });
      const { result, data, count, msg } = res.data;
      debugger;
      if (result) {
    ...
    async getTableHeader() {
      let IDs = [{ ID: 1200 }, { ID: 1201 }, { ID: 7930 }];
    ...
    openDrawer() {
      ...
      this.ruleForm0["RowNumber"] = 111;
      this.ruleForm0["OperationStatus"] = -1;

      this.ruleForm0["DeliveryPlanDate"] =
        new Date().getFullYear() +
        "-" +
        (new Date().getMonth() + 1) +
        "-" +
    ...
    addGoods(val) {
      ...
      if (data != null) {
        data.forEach((m) => {
          ...
          m1["Extend2"] = "";
          m1["DeliveryPlanID"] = "";
          m1["RowNumber"] = this.RowNumber;
          this.tableData[1].push(m1);
          this.RowNumber += 1;
        ...
        if (valid) {
          ...
          if (_this.editTag == 0) {
            ...
            if (childrens.length != 0) {
              childrens.forEach((x) => {
                x.dicID = 1201;
                x["DeliveryPlanHeadID"] = "";
                if (parseInt(x.DeliveryQty) <= 0) {
                  flag = 1;
                }
              });
            }
            ...
            if (_this.tableData[1].length != 0) {
              ...
              _this.tableData[1].forEach((x) => {
                if (!x.ReceiptDetailID) {
                  x.DeliveryPlanHeadID = this.ruleForm0.DeliveryPlanHeadID;
                  x.dicID = 1201;
                  newData1.push(x);
                }
              });
              newData2 = _this.templateData.filter(
                (c) =>
                  !_this.tableData[1].some(
                    (z) => c.DeliveryPlanID == z.DeliveryPlanID
                  )
              ); //删除
    ...
    // 打印
    async printData() {
      this.$refs["lodop"].ProgramCodesTMP = null;
      let obj = {
        row: 0,
        page: 1,
        ...
        dicID: 1201,
        TemplateID: "P001",
        DeliveryPlanHeadID: this.ruleForm0.DeliveryPlanHeadID,
      };
    ...
    async GetPrintProgramCodes() {
      let res = await GetPrintProgramCodes({
        ID: this.ruleForm0.DeliveryPlanHeadID,
        ProjectCode: "P002",
        TemplateCode: "T002",
      });
    ...
    computedmoney(row, val, prop, index) {
      if (prop == "DeliveryQty") {
        let newStr = this.computedHunfrend(val);
        row.Extend2 = newStr;
        this.$set(row, "Extend2", newStr);
      }
      // if (row.Price && row.DeliveryQty) {
      //   let money = "";
      //   money = parseFloat(row.Price) * parseFloat(row.DeliveryQty);
    ...
    //格式化数据
    computedHunfrend(money) {
      const temp = money.split(".");
      return (
        temp[0].replace(/(\d)(?=(\d{3})+$)/g, "$1,") + "." + (temp[1] || "0")
      );
    },
  },
};
