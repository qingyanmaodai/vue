        ...
        <div class="ant-table-title">
          <el-row>
            ...
            <el-col :span="20" class="flex_flex_end">
              <el-button type="warning" size="mini" @click="ResetSorting"
                >重置排序</el-button
              >
              <el-divider direction="vertical"></el-divider>
              <!-- <div  v-for="(item, y) in Status1" :key="y">
                <span @click="changeStatus(item, y)" :class="labelStatus1 == y ? 'statusActive cursor' : 'cursor'">{{ item.OrganizeName }}</span>
                <el-divider direction="vertical"></el-divider>
              </div> -->
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    setData() {
      ...
      this.spread.bind(GCsheets.Events.ClipboardPasted, function (s, e) {
        // 日期列才触发
        if (
          _this.tableColumns[_this.tagRemark][e.cellRange.col].prop.indexOf(
            "-"
          ) > -1
        ) {
          // 正则分割剪切单元格的所有值
            ...
            for (let r = 0; r < rowIndex; r++) {
              ...
              for (let c = 0; c < colIndex; c++) {
                ...
                _this.computedNum(
                  row,
                  col,
                  /^[0-9]+.?[0-9]*$/.test(cellValList[num])
                    ? parseInt(cellValList[num])
                    : 0
                );
                // 复制的列累加
                col++;
    ...
    async ResetSorting() {
      ...
      this.$confirm("确定重置排序吗？")
        .then(async (_) => {
          res = await GetSearch(
            _this.tableData[this.tagRemark],
            "/APSAPI/ResetEKViewSort"
          );
          const { datas, forms, result, msg } = res.data;
          if (result) {
            _this.$message({
