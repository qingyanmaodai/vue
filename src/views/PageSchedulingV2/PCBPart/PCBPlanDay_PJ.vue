...
export default {
  ...
  methods: {
    ...
    setData() {
      ...
      //sheet.setFormatter(-1, 9, 'yyyy-mm-dd');
      this.tableData[0].forEach((m, index) => {
        var cellType2 = new GC.Spread.Sheets.CellTypes.ComboBox();

        cellType2.editorValueType(
          GC.Spread.Sheets.CellTypes.EditorValueType.value
        );
        cellType2.items(m.Lines);
        sheet.getCell(index, 1).cellType(cellType2);
      });
    ...
    async getLabelLineData() {
      ...
      const { result, data, count, msg } = res.data;
      if (result) {
        let newData_2 = data.filter((a) => {
          return (
            a.ProcessID != "P202009092233413" &&
            a.ProcessID != "P202009092233201"
          );
        });
        this.Status1 = newData.concat(newData_2);
      } else {
        this.$message({
          message: msg,
          type: "error",
