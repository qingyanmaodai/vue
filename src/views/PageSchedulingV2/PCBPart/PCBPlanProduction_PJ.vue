    ...
    <div>
      <div class="admin_content">
        ...
        <div v-for="(x,y) in 3" :key="y" v-show="labelStatus1 == y">
          <ComVxeTable :rowKey="'RowNumber'" :height="height" :tableData="tableData[y]" :tableHeader="tableColumns[y]"
  ...
  export default {
    ...
    methods: {
      ...
      async dataSave(remarkTb) {
       ...
       //   if (flag.length == 0) {
         let flag=true;
            for (let item of this.selectionData[1].values()) {
              item["dicID"] = 5586;
              item["ProducedDate"] = item.PlanDay;
               if (item["ProductionQty"] > item["MaxProductionQty"]) {
            ...
            let res = await SaveData(this.selectionData[1]);
            const {
              result,
              data,
              count,
      ...
      async addData(val) {
             ...
             for(var a of newData)
        {
          if (a["ProductionQty"] > a["ProcessOweQty"]) {
           ...
           if (a["ProductionQty"]==null||a["ProductionQty"]<=0) {
