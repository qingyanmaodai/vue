...
export default {
    ...
    $route: {
      handler: function (val, oldVal) {
        if (val.params.hasOwnProperty("SalesOrderDetailID")) {
          ...
          this.$set(this.formSearchs[0].datas, "dicID", 7854);
          this.dataSearch(0);
        } else {
  ...
  methods: {
    ...
    cellStyle({ row, column }) {
      if (column.property == "P") {
        if (parseFloat(row.P.substring(0, row.P.length - 1)) == 0) {
          return {
            backgroundColor: "#ff7b7b",
          };
        } else if (parseFloat(row.P.substring(0, row.P.length - 1)) >= 100) {
          return {
            backgroundColor: "#9fff9f",
          };
        } else if (parseFloat(row.P.substring(0, row.P.length - 1)) < 100) {
    ...
    handleRowClick(row, remarkTb) {
      if (remarkTb == 0) {
        ...
        this.searchForm.SalesOrderDetailID = row.SalesOrderDetailID;
        this.$set(this, "tableDialog", true);
      }
    },
    // 关闭弹框
    closeDialog(val) {
